...
public class RedisSessionManager extends ManagerBase implements Lifecycle {
  ...
  public Session findSession(String id) throws IOException {
    ...
    if (id == null) {
      ...
      log.fine("id.equals(currentSessionId.get()) >>>>>>>>>>>>>>>>>>>>>>>>>>");
      session = currentSession.get();
    } else {
      session = loadSessionFromRedis(id);
      
      
      log.fine("loadSessionFromRedis(id)>>>>>>:" + "id:" + id + "------session:" + session);
  ...
  public void save(Session session) throws IOException {
    ...
    try {
      ...
      if (sessionIsDirty || currentSessionIsPersisted.get() != true) {
        ...
        log.fine("jedis.set(binaryId, serializer.serializeFrom(redisSession))>>>>:"+ "binaryId:" + binaryId +
        		"--------redisSession:" + redisSession); 
        
        /*
         * 避免redis中存储的session有效期在短时间内重复刷新致使redis岩机
         */
        Long  expireTime = jedis.ttl(binaryId);
        
        log.fine("jedis.ttl(id.getBytes())>>>>:" + "binaryId:" + binaryId + 
        		  "-------getMaxInactiveInterval:" + getMaxInactiveInterval() + "---------expireTime:" + expireTime);
        if(expireTime < 0 || (getMaxInactiveInterval() - expireTime) > 10 * 60){
          /*
           * 1、首次进入的请求session在redis的有效期赋予初值   2、超过10分钟请求session存储到redis的有效时间刷新
           */
        	jedis.expire(binaryId, getMaxInactiveInterval());
      	  
        	log.fine("jedis.expire(binaryId, getMaxInactiveInterval())>>>>:" + "binaryId:" + binaryId + 
